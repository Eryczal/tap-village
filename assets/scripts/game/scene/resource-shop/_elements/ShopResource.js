...
class ShopResource extends Element {
	constructor(game, parent, id, column, row, type) {
		...
		switch (this.type) {
			...
			case 1:
				this.amount = Math.floor(resources[this.resourceId].amount * (QuarryBuilding.stats.gatheringPower + 1) * (QuarryBuilding.stats.gatheringChance / 100));
				break;

			case 2:
				this.amount = Math.floor(resources[this.resourceId].amount * (MineBuilding.stats.gatheringPower + 1) * (MineBuilding.stats.gatheringChance / 100));
				break;
			...
			case 3:
				...
				let gold = Math.floor(resources[this.resourceId].amount * 0.4 * (MineBuilding.stats.gatheringPower + 1) * (MineBuilding.stats.gatheringChance / 100));
